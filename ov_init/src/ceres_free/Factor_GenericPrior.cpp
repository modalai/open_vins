 ...
 *
 * Lifted from ov_init/src/ceres/Factor_GenericPrior.cpp (residual + Jacobians verbatim).
 *
  ...
  for (size_t i = 0; i < x_type.size(); i++) {
    if (x_type[i] == "quat") {
      ...
      if (jacobians && jacobians[i]) {
        ...
        // NOTE: correctness fix vs upstream ov_init/src/ceres/Factor_GenericPrior.cpp, which
        // assigned the (N x 3) product into an (N x 1) block. That mismatch is silently
        // truncated to one column under -DNDEBUG (Release) -> a wrong yaw-prior Jacobian, and
        // aborts in a debug build. The yaw error maps from all 3 tangent dofs (H_theta is 1x3),
        // so the destination must be the 3 tangent columns of the ambient-4 quaternion block.
