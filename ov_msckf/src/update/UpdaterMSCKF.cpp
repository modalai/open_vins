  ...
  for (const auto &clone_calib : state->_calib_IMUtoCAM) {
    ...
    for (const auto &clone_imu : state->_clones_IMU) {
      ...
      if (std::abs(dt_cam_delta) > 1e-10) {
        if (state->_clones_kinematics.find(clone_imu.first) == state->_clones_kinematics.end()) {
          fprintf(stderr, "[DIAG] UpdaterMSCKF: _clones_kinematics missing clone_time=%.6f, cam_id=%zu, dt_cam_delta=%.6e, #kin=%zu, #clones=%zu\n",
                  clone_imu.first, clone_calib.first, dt_cam_delta, state->_clones_kinematics.size(), state->_clones_IMU.size());
        }
        const State::CloneKinematics &kin = state->_clones_kinematics.at(clone_imu.first);
        R_GtoIi = (Eigen::Matrix3d::Identity() - skew_x(kin.omega) * dt_cam_delta) * R_GtoIi;
        p_IiinG = p_IiinG + kin.vel * dt_cam_delta;
      }
