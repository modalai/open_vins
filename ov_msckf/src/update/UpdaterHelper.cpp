...
void UpdaterHelper::get_feature_jacobian_representation(std::shared_ptr<State> state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
  ...
  // Rolling shutter readout for anchor camera
  if (state->_calib_camera_readout.find(feature.anchor_cam_id) == state->_calib_camera_readout.end()) {
    fprintf(stderr, "[DIAG] get_feature_jacobian_representation: _calib_camera_readout missing anchor_cam_id=%d, map.size()=%zu\n",
            feature.anchor_cam_id, state->_calib_camera_readout.size());
  }
  ...
  if (need_anchor_correction) {
    ...
    if (state->_clones_kinematics.find(feature.anchor_clone_timestamp) == state->_clones_kinematics.end()) {
      fprintf(stderr, "[DIAG] get_feature_jacobian_representation: _clones_kinematics missing anchor_clone_timestamp=%.6f, cam_id=%d, #kin=%zu, #clones=%zu\n",
              feature.anchor_clone_timestamp, feature.anchor_cam_id, state->_clones_kinematics.size(), state->_clones_IMU.size());
    }
    const State::CloneKinematics &kin_anc = state->_clones_kinematics.at(feature.anchor_clone_timestamp);
...
void UpdaterHelper::get_feature_jacobian_full(std::shared_ptr<State> state, UpdaterHelperFeature &feature, Eigen::MatrixXd &H_f,
  ...
  if (LandmarkRepresentation::is_relative_representation(feature.feat_representation)) {
    ...
    double dt_camoff_anc = state->cam_imu_dt_delta(feature.anchor_cam_id);
    if (state->_calib_camera_readout.find(feature.anchor_cam_id) == state->_calib_camera_readout.end()) {
      fprintf(stderr, "[DIAG] get_feature_jacobian_full: _calib_camera_readout missing anchor_cam_id=%d, map.size()=%zu\n",
              feature.anchor_cam_id, state->_calib_camera_readout.size());
    }
    ...
    if (std::abs(dt_total_anc) > 1e-10) {
      if (state->_clones_kinematics.find(feature.anchor_clone_timestamp) == state->_clones_kinematics.end()) {
        fprintf(stderr, "[DIAG] get_feature_jacobian_full(anchor): _clones_kinematics missing anchor_clone_timestamp=%.6f, cam_id=%d, dt_total_anc=%.6e, #kin=%zu, #clones=%zu\n",
                feature.anchor_clone_timestamp, feature.anchor_cam_id, dt_total_anc, state->_clones_kinematics.size(), state->_clones_IMU.size());
      }
      const State::CloneKinematics &kin_anc = state->_clones_kinematics.at(feature.anchor_clone_timestamp);
      const Eigen::Vector3d &omega_anc = kin_anc.omega;
      const Eigen::Vector3d &v_anc = kin_anc.vel;
      R_GtoI = (Eigen::Matrix3d::Identity() - skew_x(omega_anc) * dt_total_anc) * R_GtoI;
      p_IinG = p_IinG + v_anc * dt_total_anc;
    }
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    if (state->_cam_intrinsics.find(cam_id) == state->_cam_intrinsics.end() ||
        ...
        state->_cam_intrinsics_cameras.find(cam_id) == state->_cam_intrinsics_cameras.end() ||
        state->_calib_camera_readout.find(cam_id) == state->_calib_camera_readout.end()) {
      fprintf(stderr, "[DIAG] get_feature_jacobian_full(cam loop): missing state entry for cam_id=%zu, "
              "intrinsics=%d, calib=%d, cam_model=%d, readout=%d\n",
              cam_id,
              (int)(state->_cam_intrinsics.count(cam_id)),
              (int)(state->_calib_IMUtoCAM.count(cam_id)),
              (int)(state->_cam_intrinsics_cameras.count(cam_id)),
              (int)(state->_calib_camera_readout.count(cam_id)));
    }
    ...
    if (state->_options.do_calib_camera_readout) {
      if (map_hx.find(readout) == map_hx.end()) {
        fprintf(stderr, "[DIAG] get_feature_jacobian_full: map_hx missing readout for cam_id=%zu, map_hx.size()=%zu\n", cam_id, map_hx.size());
      }
    ...
    for (size_t m = 0; m < cam_timestamps.size(); m++) {
      ...
      const Eigen::Vector3d *omega_clone_val = nullptr;
      const Eigen::Vector3d *v_clone_val = nullptr;
      const Eigen::Vector3d *omega_clone_lin = nullptr;
      const Eigen::Vector3d *v_clone_lin = nullptr;
      double v_pixel = (double)cam_uvs.at(m)(1);
      if (need_rs_terms) {
        double clone_time = cam_timestamps.at(m);
        if (state->_clones_kinematics.find(clone_time) == state->_clones_kinematics.end()) {
          fprintf(stderr, "[DIAG] get_feature_jacobian_full(obs): _clones_kinematics missing clone_time=%.6f, cam_id=%zu, meas=%zu/%zu, #kin=%zu, #clones=%zu\n",
                  clone_time, cam_id, m, cam_timestamps.size(), state->_clones_kinematics.size(), state->_clones_IMU.size());
        }
        const State::CloneKinematics &kin = state->_clones_kinematics.at(clone_time);
        omega_clone_val = &kin.omega;
        v_clone_val = &kin.vel;
        if (state->_options.do_fej) {
          omega_clone_lin = &kin.omega_fej;
          v_clone_lin = &kin.vel_fej;
        } else {
      ...
      if (state->_options.do_calib_camera_readout) {
        assert(omega_clone_lin != nullptr && v_clone_lin != nullptr);
      ...
      // Derivative of measurement in respect to camera-imu timeoffsets
      if (state->_options.do_calib_camera_timeoffset && dt_cam_hx_col >= 0 && dt_ref_hx_col >= 0 && dt_cam_hx_col != dt_ref_hx_col) {
        assert(omega_clone_lin != nullptr && v_clone_lin != nullptr);
