...
void StateHelper::EKFPropagation(
    ...
    for (size_t i = 0; i < order_NEW.size() - 1; i++) {
        if (order_NEW.at(i)->id() + order_NEW.at(i)->size() != order_NEW.at(i + 1)->id()) {
            ...
            PRINT_ERROR(RED "StateHelper::EKFPropagation() - This code only support a state transition which is in the same order as the "
                            "state\n" RESET);
            std::exit(EXIT_FAILURE);
        }
...
//            printf("state->_features_SLAM.erase  1\n");
