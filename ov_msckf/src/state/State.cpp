...
State::State(StateOptions &options) {
  ...
  for (int i = 0; i < _options.num_cameras; i++) {
    ...
    // Allocate camera-imu time offset (1 DOF per camera)
    ...
    // Allocate rolling shutter readout time (1 DOF per camera)
  ...
  if (_calib_dt_CAMtoIMU == nullptr && !_calib_dt_CAMtoIMU_map.empty()) {
    _calib_dt_CAMtoIMU = _calib_dt_CAMtoIMU_map.begin()->second;
  }
