...
void Propagator::propagate_only(std::shared_ptr<State> state, double timestamp) {
...
std::vector<ov_core::ImuData> Propagator::select_imu_readings(const std::vector<ov_core::ImuData> &imu_data, double time0, double time1,
    ...
    // Ensure we have some measurements in the first place!
    if (imu_data.empty()) {
         printf(YELLOW "Propagator::select_imu_readings(): START LOGIC--No IMU measurements. IMU-CAMERA are likely messed up!!!\n" RESET);
        return prop_data;
    }
    ...
    for (size_t i = 0; i < imu_data.size() - 1; i++) {
        ...
        if (imu_data.at(i + 1).timestamp > time0 && imu_data.at(i).timestamp < time0) {
            ov_core::ImuData data = Propagator::interpolate_data(imu_data.at(i), imu_data.at(i + 1), time0);
            prop_data.push_back(data);
             printf("propagation #%d = CASE 1 = %.3f => %.3f %d\n", (int)i, data.timestamp - prop_data.at(0).timestamp,
                         time0 - prop_data.at(0).timestamp, imu_data.size());
            continue;
        }
        ...
        if (imu_data.at(i).timestamp >= time0 && imu_data.at(i + 1).timestamp <= time1) {
            prop_data.push_back(imu_data.at(i));
            printf("propagation #%d = CASE 2 = %.3f %d\n", (int)i, imu_data.at(i).timestamp - prop_data.at(0).timestamp, imu_data.size());
            continue;
        }
        ...
        if (imu_data.at(i + 1).timestamp > time1) {
            // If we have a very low frequency IMU then, we could have only recorded the first integration (i.e. case 1) and nothing else
            // In this case, both the current IMU measurement and the next is greater than the desired intepolation, thus we should just cut
            // the current at the desired time Else, we have hit CASE2 and this IMU measurement is not past the desired propagation time,
            // thus add the whole IMU reading
            if (imu_data.at(i).timestamp > time1 && i == 0) {
                // This case can happen if we don't have any imu data that has occured before the startup time
                // This means that either we have dropped IMU data, or we have not gotten enough.
                // In this case we can't propgate forward in time, so there is not that much we can do.
            	printf("imu_data.at(i).timestamp > time1 && i == 0 don't have any imu data that has occured before the startup time\n");
                break;
            } else if (imu_data.at(i).timestamp > time1) {
                ov_core::ImuData data = interpolate_data(imu_data.at(i - 1), imu_data.at(i), time1);
                prop_data.push_back(data);
...
//        else
//        	printf("imu_data.at(i + 1).timestamp < time1, outside period\n");

    }

    // Check that we have at least one measurement to propagate with
    if (prop_data.empty()) {
            printf(YELLOW "Propagator::select_imu_readings(): No IMU measurements to propagate with (%d of 2). IMU-CAMERA are "
                                 "likely messed up!!!\n" RESET,
                          (int)prop_data.size());
        return prop_data;
    }
