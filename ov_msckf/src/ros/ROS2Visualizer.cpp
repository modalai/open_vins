...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  if (_app->get_params().use_multi_threading_pubs) {
    image_publisher = std::thread([this] {
      ...
      while (!stop_image_publisher.load(std::memory_order_acquire) && rclcpp::ok()) {
...
void ROS2Visualizer::callback_stereo(const sensor_msgs::msg::Image::ConstSharedPtr msg0, const sensor_msgs::msg::Image::ConstSharedPtr msg1,
  ...
  camera_last_timestamp[cam_id0] = timestamp;
  const double timestamp1 = cv_ptr1->header.stamp.sec + cv_ptr1->header.stamp.nanosec * 1e-9;
...
void ROS2Visualizer::publish_loopclosure_information() {
  ...
  if (pub_loop_pose->get_subscription_count() != 0 || pub_loop_extrinsic->get_subscription_count() != 0 ||
    ...
    odometry_pose.pose.pose.position.x = active_pose->pos()(0);
    odometry_pose.pose.pose.position.y = active_pose->pos()(1);
    odometry_pose.pose.pose.position.z = active_pose->pos()(2);
    odometry_pose.pose.pose.orientation.x = active_pose->quat()(0);
    odometry_pose.pose.pose.orientation.y = active_pose->quat()(1);
    odometry_pose.pose.pose.orientation.z = active_pose->quat()(2);
    odometry_pose.pose.pose.orientation.w = active_pose->quat()(3);
