...
void ROS1Visualizer::visualize_final() {
  ...
  if (_app->get_state()->_options.do_calib_camera_timeoffset) {
    for (int i = 0; i < _app->get_state()->_options.num_cameras; i++) {
      PRINT_INFO(REDPURPLE "cam%d-imu timeoffset = %.5f\n" RESET, i, _app->get_state()->cam_imu_dt((size_t)i));
    }
    PRINT_INFO("\n");
  }
  ...
  std::thread thread([&] {
    ...
    if (unique_cam_ids.size() == num_unique_cameras) {
      ...
      while (!camera_queue.empty()) {
        ...
        double timestamp_imu_of_image = cam_msg.timestamp + _app->get_state()->cam_imu_dt_max_for_ids(cam_msg.sensor_ids);
        if (timestamp_imu_of_image >= message.timestamp) {
          break;
        }
        auto rT0_1 = boost::posix_time::microsec_clock::local_time();
        double update_dt = 100.0 * (message.timestamp - timestamp_imu_of_image);
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  // We want to publish in the IMU clock frame
  // The timestamp in the state will be the last camera time
  double t_ItoC = _app->get_state()->cam_imu_dt_ref();
