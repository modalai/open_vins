...
void ROSVisualizerHelper::sim_save_total_state_to_file(std::shared_ptr<State> state, std::shared_ptr<Simulator> sim,
  ...
  of_state_est << state->cam_imu_dt_ref() << " ";
  ...
  if (state->_options.do_calib_camera_timeoffset) {
    of_state_std << std::sqrt(cov(state->cam_imu_dt_var((size_t)state->cam_imu_dt_ref_camid())->id(),
                                  state->cam_imu_dt_var((size_t)state->cam_imu_dt_ref_camid())->id()))
                 << " ";
  } else {
    of_state_std << 0.0 << " ";
  }
