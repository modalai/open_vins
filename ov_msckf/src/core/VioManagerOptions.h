...
struct VioManagerOptions {
    ...
    void print_and_load_state(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
        ...
        for (int n = 0; n < state_options.num_cameras; n++) {
            std::stringstream ss;
            ss << "cam_" << n << "_fisheye:" << (std::dynamic_pointer_cast<ov_core::CamEqui>(camera_intrinsics.at(n)) != nullptr)
               << std::endl;
    ...
    /// Measurement noise seed. This should be incremented for each run in the Monte-Carlo simulation to generate the same true
    /// measurements, but diffferent noise values.
    ...
    /// We will start simulating after we have moved this much along the b-spline. This prevents static starts as we init from groundtruth
    /// in simulation.
