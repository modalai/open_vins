...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
    ...
    if (state->_timestamp > message.timestamp) {
        PRINT_WARNING(YELLOW "%s image received out of order, unable to do anything (prop dt = %3f)\n" RESET,
                      message.images.size() == 2 ? "stereo" : "mono", (message.timestamp - state->_timestamp));
        return;
    }
    ...
    if (state->_options.do_calib_camera_pose) {
        for (int i = 0; i < state->_options.num_cameras; i++) {
            ...
            PRINT_INFO("cam%d extrinsics = %.3f,%.3f,%.3f,%.3f | %.3f,%.3f,%.3f\n", (int)i, calib->quat()(0), calib->quat()(1),
                       calib->quat()(2), calib->quat()(3), calib->pos()(0), calib->pos()(1), calib->pos()(2));
        }
    }
}
