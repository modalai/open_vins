...
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {
  ...
  for (int i = 0; i < state->_options.num_cameras; i++) {
    // Timeoffset from camera to IMU (per camera)
...
void VioManager::feed_measurement_simulation(double timestamp, const std::vector<int> &camids,
  ...
  if (is_initialized_vio && updaterZUPT != nullptr && (!params.zupt_only_at_beginning || !has_moved_since_zupt)) {
    ...
    if (did_zupt_update) {
      ...
      double dt_max = state->cam_imu_dt_max_for_ids(camids);
      propagator->clean_old_imu_measurements(timestamp + dt_max - params.prop_window);
      updaterZUPT->clean_old_imu_measurements(timestamp + dt_max - params.prop_window);
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  if (is_initialized_vio && updaterZUPT != nullptr && (!params.zupt_only_at_beginning || !has_moved_since_zupt)) {
    ...
    if (did_zupt_update) {
      ...
      double dt_max = state->cam_imu_dt_max_for_ids(message.sensor_ids);
      propagator->clean_old_imu_measurements(message.timestamp + dt_max - params.prop_window);
      updaterZUPT->clean_old_imu_measurements(message.timestamp + dt_max - params.prop_window);
      propagator->invalidate_cache();
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  // Print rolling shutter readout and per-camera time offset values
  {
    std::stringstream ss_rs;
    ss_rs << "[RS/DT]:";
    for (int i = 0; i < 1; i++) {
      double t_rd = state->_calib_camera_readout.at(i)->value()(0);
      double t_dt = state->cam_imu_dt(i);
      ss_rs << " cam" << i << "(readout=" << std::setprecision(6) << t_rd
            << ", t_dt=" << std::setprecision(6) << t_dt << ")";
    }
    printf("%s\n", ss_rs.str().c_str());
  }
