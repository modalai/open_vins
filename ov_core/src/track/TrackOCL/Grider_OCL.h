  ...
  public:
    ...
    static void perform_griding(OCLTracker* tracker, const cv::Mat &img, const cv::Mat &mask, const std::vector<std::pair<int, int>> &valid_locs,
        ...
        for (int r = 0 ; r < (int)valid_locs.size(); r++)
        {
            ...
            size_t offset = (max_keypoints * 3 + 1) * sizeof(int) * 0;
            ...
            size_t work[] = { (size_t)tracker->detection_buf.max_points };
    ...
    static void perform_griding_gpu(OCLTracker* tracker, const cl_mem img, const cv::Mat &mask, int w, int h, const std::vector<std::pair<int, int>> &valid_locs,
                                std::vector<cv::KeyPoint> &pts, int num_features, int grid_x, int grid_y, int threshold,
                                bool nonmaxSuppression) {
        ...
        for (int r = 0 ; r < (int)valid_locs.size(); r++)
        {
            ...
            // Skip if we are out of bounds
            if (x + size_x > w || y + size_y > h) continue;
            ...
            err |= clSetKernelArg(tracker->extract_whole_img_kernel, 10, sizeof(int),    &tracker->detection_buf.max_points);
            ...
            size_t global_work_size[2] = { size_x - 6, size_y - 6 };
            err = clEnqueueNDRangeKernel(tracker->queue, tracker->extract_whole_img_kernel, 2, NULL, global_work_size, NULL, 0, NULL, NULL);
            if (err != CL_SUCCESS) {
                printf("Error running extract_kernel: %d\n", err);
                return;
            }
            ...
            err |= clSetKernelArg(tracker->nms_kernel, 3, sizeof(int),    &step);
            err |= clSetKernelArg(tracker->nms_kernel, 4, sizeof(int),    &img_offset);
            err |= clSetKernelArg(tracker->nms_kernel, 5, sizeof(int),    &h);
            err |= clSetKernelArg(tracker->nms_kernel, 6, sizeof(int),    &w);
            err |= clSetKernelArg(tracker->nms_kernel, 7, sizeof(int),    &counter);
            err |= clSetKernelArg(tracker->nms_kernel, 8, sizeof(int),    &tracker->detection_buf.max_points);
            if (err != CL_SUCCESS) {
                printf("Error setting nms_kernel arg: %d\n", err);
    ...
    static void perform_griding_gpu2(OCLTracker* tracker, const cl_mem img, const cv::Mat &mask, int w, int h, const std::vector<std::pair<int, int>> &valid_locs,
                                std::vector<cv::KeyPoint> &pts, int num_features, int grid_x, int grid_y, int threshold,
                                bool nonmaxSuppression) {
        ...
        err |= clSetKernelArg(tracker->nms_kernel, 5, sizeof(int),    &h);
        err |= clSetKernelArg(tracker->nms_kernel, 6, sizeof(int),    &w);
        err |= clSetKernelArg(tracker->nms_kernel, 8, sizeof(int),    &max_keypoints);
        ...
        for (int r = 0 ; r < (int)valid_locs.size(); r++)
        {
            ...
            size_t global_work_size[2] = { size_x - 6, size_y - 6 };
            err = clEnqueueNDRangeKernel(tracker->queue, tracker->extract_whole_img_kernel, 2, NULL, global_work_size, NULL, 0, NULL, &keypoints_events[r]);
            if (err != CL_SUCCESS) {
                printf("Error running extract_kernel: %d\n", err);
                return;
            }

            size_t offset = (max_keypoints * 3 + 1) * sizeof(int) * r;
            ...
            err = clEnqueueNDRangeKernel(tracker->queue, tracker->nms_kernel, 1, NULL, work, NULL, 1, &keypoints_events[r], &nonmax_events[r]);
            if (err != CL_SUCCESS) {
                printf("Error running nms_kernel: %d\n", err);
                return;
            }
        ...
        int* kp_out = (int*)malloc((max_keypoints * 3 + 1) * sizeof(int) * (int)valid_locs.size());

        clEnqueueReadBuffer(tracker->queue, tracker->detection_buf.xyz_pts_buf, CL_FALSE, 0, (max_keypoints * 3 + 1) * sizeof(int) * (int)valid_locs.size(), kp_out, nonmax_events.size(), nonmax_events.data(), &read_event);
