...
void TrackSIM::feed_measurement_simulation(double timestamp, const std::vector<int> &camids,
  ...
  for (size_t i = 0; i < camids.size(); ++i)
    feed_measurement_simulation(timestamp, camids.at(i), feats.at(i));
}

void TrackSIM::feed_measurement_simulation(double timestamp, int cam_id, const FeatureObservations &features) {
